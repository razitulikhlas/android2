package com.example.razitulikhlas.projectdicoding;

import android.content.Context;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;
import android.provider.BaseColumns;

public class DBpelajaran extends SQLiteOpenHelper {

    static abstract class MyColumns implements BaseColumns {
        //Menentukan Nama Table dan Kolom
        static final String NamaTabel = "Jadwal";
        static final String Nama = "Nama_Dosen";
        static final String Matakuliah = "Matakuliah";
        static final String Ruangan = "Ruangan";
        static final String Waktu = "Waktu";
        static final String Hari = "Hari";

    }

    private static final String NamaDatabse = "rpl.db";
    private static final int VersiDatabase = 14;

    private static final String SQL_CREATE_ENTRIES = "CREATE TABLE "+MyColumns.NamaTabel+
            "("+MyColumns.Nama+" TEXT , "+
            MyColumns.Hari+" TEXT NOT NULL, "+MyColumns.Matakuliah+
            " TEXT NOT NULL, "+MyColumns.Ruangan+" TEXT NOT NULL, "+MyColumns.Waktu+
            " TEXT NOT NULL)";

    //Query yang digunakan untuk mengupgrade Tabel
    private static final String SQL_DELETE_ENTRIES = "DROP TABLE IF EXISTS "+MyColumns.NamaTabel;

    DBpelajaran(Context context) {
        super(context, NamaDatabse, null, VersiDatabase);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(SQL_CREATE_ENTRIES);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int i, int i1) {
        db.execSQL(SQL_DELETE_ENTRIES);
        onCreate(db);
    }
}
